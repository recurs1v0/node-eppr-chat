...
# node-eppr-chat
Inicia una sala de chat local adentro de tu Raspberry Pi.

En este proyecto utilizamos un par de módulos nuevos para hacer fácil la integración con dispositivos móviles.

Utilizamos Socket.IO para montar la sala de chat privada.
...
```
sudo curl -sSL git.eppr.link/chat | sh
```
...
```
sudo npm install --save express ejs qrcode-terminal moment socket.io
```
...
Una sala de chat no es lo mismo si estás solo, ingresa desde varios dispositivos móviles conectados a la misma red Wifi.
